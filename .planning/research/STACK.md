...
**Domain:** High-performance Linux X11 → Apple Silicon macOS remote desktop
**Researched:** 2026-07-26
**Confidence:** MEDIUM overall (HIGH for inspected Kyber pins and public NVIDIA limits; MEDIUM for macOS codec paths until hardware spikes)

## Recommendation

Fork **Kyber Desktop 0.27.0 exactly**, preserve its recursive dependency graph, and patch it narrowly. Do not “modernize” its Rust, FFmpeg, VLC, QUIC, winit, or NvFBC dependencies before the 4K60 baseline is reproducible. The current upstream is unusually fresh (released 2026-06-26) and already carries coordinated FFmpeg/VLC/transport patches; dependency churn would make performance failures ambiguous.

Use the existing Kyber/Kymux Rust control and input plane, Kyber `txproto` capture/encode graph, NvFBC shared-CUDA capture, FFmpeg NVENC, and Kyber's libVLC client. Add small native macOS C/Objective-C bridges for VideoToolbox capability/session probes, CoreGraphics immersive input, and NSPasteboard. Keep AV1 experimental until the pinned VLC/FFmpeg path proves VideoToolbox decode on an actual M3-or-newer Mac.
...
Record these in `UPSTREAM.lock` and verify them in CI with `git submodule status --recursive`:
...
Also retain every Meson wrap URL and SHA-256. Important existing pins include libplacebo `7.351.0`, glslang `16.0.0`, Vulkan headers/loader/SPIR-V `1.4.313`, x264 `a8b68ebfaa68621b5ac8907610d3335971839d52`, zlib `1.3.2`, libogg `1.3.5`, and Opus `1.3.1`. Audio is disabled at runtime but these remain build inputs until a later dependency-pruning phase.
...
Run probes before presenting a codec/chroma choice. Cache results only for the current GPU UUID + NVIDIA driver + Mac model/chip + OS build + app build.
...
1. **Immutable baseline:** vendor no binaries from the network at build time. Mirror source archives, verify recorded SHA-256 values, initialize exact gitlinks, and build with `--locked`/Meson wrap lock behavior.
2. **Patch queue:** one topic per commit under a documented series: security/trust, capabilities/negotiation, telemetry, macOS decoder reporting, immersive input, clipboard. Do not mix upstream refreshes with feature patches.
3. **Dual lanes:** `baseline` stays on 0.27.0 pins through the 4K60 gate; `qualification` may test newer NVIDIA/Xcode/macOS betas. Promote an upgrade only with identical fixtures and no latency/fidelity regression.
4. **System ABI boundary:** never bundle NVIDIA libraries or glibc. At startup record and validate kernel, driver module, userspace NVIDIA library, Xorg, GPU UUID/architecture, display mode, and permissions.
5. **License boundary:** distribute the complete corresponding source, build scripts, local patches, notices, and install information for the AGPL-covered network program. Treat future proprietary distribution as a separate commercial-license and codec-license decision.
...
---
*Stack research for: LinuxRemote*
*Researched: 2026-07-26*
