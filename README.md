...
This plan deliberately implements no native readiness proof. Local Xorg,
NVML, selected output mapping, NvFBC capture, and NVENC tuples remain
`UNPROVEN`, so the current command cannot produce G0 PASS.
...
- prove an active local physical Xorg session and live NVIDIA identity through
  NVML;
