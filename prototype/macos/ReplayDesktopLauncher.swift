...
private enum KyclientArguments {
    ...
    static func build(for configuration: LauncherConfiguration) throws -> [String] {
        ...
        arguments.append("--keyboard-grab=\(configuration.keyboardGrabEnabled)")
...
private enum PreferenceKey {
    ...
    static let keyboardGrabEnabled = "prototype.keyboardGrabEnabled"
}
...
private final class LauncherController: NSObject, NSTextFieldDelegate, NSWindowDelegate {
    ...
    private let inputsCheckbox = NSButton(
        checkboxWithTitle: "Forward mouse and keyboard input",
        target: nil,
        action: nil
    )
    private let keyboardGrabCheckbox = NSButton(
        checkboxWithTitle: "Forward keyboard while the session window is focused",
        target: nil,
        action: nil
    )
    ...
    private func configureControls() {
        ...
        keyboardGrabCheckbox.target = self
        keyboardGrabCheckbox.action = #selector(controlChanged(_:))
    ...
    private func buildInterface() {
        ...
        addRow(
            ...
            detail: "Maps directly to --inputs."
        )
        addRow(
            to: root,
            label: "Keyboard behavior",
            control: keyboardGrabCheckbox,
            detail: "Focused-window forwarding only; macOS system shortcuts are not suppressed."
        )
    ...
    private func loadPreferences() {
        ...
        defaults.register(defaults: [
            ...
            PreferenceKey.keyboardGrabEnabled: true,
        ])
        ...
        keyboardGrabCheckbox.state =
            defaults.bool(forKey: PreferenceKey.keyboardGrabEnabled) ? .on : .off
    }
    ...
    private func savePreferences(_ configuration: LauncherConfiguration) {
        ...
        defaults.set(
            configuration.keyboardGrabEnabled,
            forKey: PreferenceKey.keyboardGrabEnabled
        )
    }
    ...
    private func configurationFromControls() throws -> LauncherConfiguration {
        ...
        return LauncherConfiguration(
            ...
            keyboardGrabEnabled: keyboardGrabCheckbox.state == .on
        )
    }
...
private struct DryRunOptions {
    ...
    init(arguments: [String]) throws {
        for argument in arguments where argument != "--dry-run" {
            ...
            case "keyboard-grab":
                keyboardGrabEnabled = try parseBoolean(value, option: name)
            case "clipboard":
                throw LauncherError(
    ...
    var configuration: LauncherConfiguration {
        get throws {
            LauncherConfiguration(
                ...
                keyboardGrabEnabled: keyboardGrabEnabled
            )
        }
    }
}
    ...
    print(
        ...
        Script modes:
          ...
          --dry-run [--host=value] [--port=value]
                    ...
                    [--inputs=true|false] [--keyboard-grab=true|false]
