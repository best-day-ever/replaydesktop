...
#[test]
fn host03_diagnostic_no_source_provider_cannot_claim_capture() {
    let evidence = replay_host_doctor::capture_one_frame(
        &replay_host_doctor::LiveUnavailableCaptureProvider,
        None,
    );
    assert_eq!(
        evidence.admission,
        replay_host_doctor::CaptureAdmissionV1::Rejected
    );
    assert_eq!(
        evidence.failure,
        Some(replay_host_doctor::CaptureFailureV1::SourceUnavailable)
    );
